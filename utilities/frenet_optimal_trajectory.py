    ...
    def run_step_single_path(self, ego_state, idx, df_n=0, Tf=4, Vf_n=0):
        ...
        if abs(d) < 1.2:
            ...
            if 0 <= df_n < len(df_values):
                ...
                df = 0
            _df = np.clip(df * self.LANE_WIDTH + d, -2.1 * self.LANE_WIDTH, 1.1 * self.LANE_WIDTH).item()
            df = closest([0.5 * self.LANE_WIDTH * lane_n for lane_n in range(-4, 3)], _df)
        else:
...
def main():
    ...
    psi = 0 # math.pi
    max_s = 50
    init_route = np.empty((0, 3))
    x = 0
    y = 0
    z = 0
    for i in range(20):
        init_route = np.append(init_route, [[x, y, z]], axis=0)
        x += 2
    # init_route = np.load('/fzi/ids/jk639/Jing_ws/rl_rule_exceptions/dreamerv3-torch/road_map/global_route_town01.npy')
    ...
    for i in range(len(init_route)):
        ax.plot(init_route[i][0], init_route[i][1], 'go', markersize=2)
    planner = FrenetPlanner()
    max_s = planner.start(init_route)

    ego_state = [x0, y0, speed, acc, psi, temp, max_s]
    ...
    ax.plot(a, d, "k-")
    for i in range(7):
        path, _, _ = planner.run_step_single_path(ego_state=ego_state, idx=1, df_n=df, Tf=4, Vf_n=-1)
